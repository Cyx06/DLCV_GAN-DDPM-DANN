...
def train_mnist():
    args = parser.arg_parse() # added
    # hardcoding these here
    n_epoch = 40
    batch_size = 256
    n_T = 400 # 500
    device = 'cuda' if torch.cuda.is_available() else 'cpu' # changed
    n_classes = 10
    ...
    ddpm.load_state_dict(torch.load(args.test))
    print(ddpm)
    ...
    dataset = DigitDataset(root=args.train_data, type=args.type, mode="train")
    dataloader = DataLoader(dataset, batch_size=args.train_batch, num_workers=args.num_workers, shuffle=True)
        ...
        with torch.no_grad():
            ...
            for w_i, w in enumerate(ws_test):
                ...
                fig, axs = plt.subplots(nrows=int(n_sample / n_classes), ncols=n_classes, sharex=True,
                                        sharey=True, figsize=(8, 3))
                print(axs.shape)

                print(x_gen_store.shape)
                print(type(x_gen_store))
                def animate_diff(i, x_gen_store):
                    ...
                    for row in range(int(n_sample / n_classes)):
                        for col in range(n_classes):
                            ...
                            print("check temp")
                            temp = np.ndarray(shape=(3, 28, 28))
                            print(temp.shape)
                            # for i in range(3):
                                # for j in range(len())
                            temp[0] = x_gen_store[i, (row * n_classes) + col, 0]
                            print(1)
                            temp[1] = x_gen_store[i, (row * n_classes) + col, 1]
                            print(2)
                            temp[2] = x_gen_store[i, (row * n_classes) + col, 2]
                            print(3)
                            plots.append(axs[row, col].imshow(temp[0]))
                            print(4)
                    """
                    for row in range(int(n_sample / n_classes)):
                        for col in range(n_classes):
                            ...
                            # the key of get gif is change vmin and vmax
                            plots.append(axs[row, col].imshow(-x_gen_store[i, (row * n_classes) + col, 0], cmap='gist_rainbow',
                                                              vmin=(x_gen).min(),
                                                              vmax=(x_gen).max()))
