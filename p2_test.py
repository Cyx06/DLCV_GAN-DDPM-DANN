...
import random
from New_model import DDPM, ContextUnet
from model import G
...
def main():
    ...
    model_G = DDPM(nn_model=ContextUnet(in_channels=3, n_feat=n_feat, n_classes=n_classes), betas=(1e-4, 0.02), n_T=n_T, device=device, drop_prob=0.1).to(device)
    model_G.load_state_dict(torch.load(args.test))
    """
    model_G = G().to(device)
    ...
    with torch.no_grad():
        ...
        fake = model_G.sample(1000, (3, 28, 28), device, guide_w=2)
        # print(x_gen_store)
        show = [None] * IMG_SHOW
        # cnt = 0
        for i in range(10):
            for j in range(100):
                if j <= 10:
                    show[10*(j-1)+i] = fake[i + j * 10]
                img = fake[i + j * 10]
                filename = os.path.join(args.save_dir, '{}_{:03d}.png'.format(i,j + 1))
